...
class TrainDataset(data.Dataset):
    ...
    def __init__(self, config):
        ...
        if not os.path.exists(train_list_file) or os.path.getsize(train_list_file) == 0:
            files = os.listdir(os.path.join(config.datasets_dir, 'label'))
    ...
    def __getitem__(self, index):
        t = cv2.imread(os.path.join(self.config.datasets_dir, 'label', str(self.imlist[index])), 1).astype(
            np.float32)
        x = cv2.imread(os.path.join(self.config.datasets_dir, 'cloud', str(self.imlist[index])), 1).astype(
            np.float32)
        n = cv2.imread(os.path.join(self.config.datasets_dir, 'nir_cloud', str(self.imlist[index])), 1).astype(
            np.float32)
...
class TestDataset(data.Dataset):
    def __init__(self, test_dir, in_ch, out_ch):
        ...
        self.in_ch = in_ch
        self.out_ch = out_ch
        self.test_files = os.listdir(os.path.join(test_dir, 'cloud'))

    def __getitem__(self, index):
        filename = os.path.basename(self.test_files[index])

        x = cv2.imread(os.path.join(self.test_dir, 'cloud', filename), 1).astype(np.float32)
        n = cv2.imread(os.path.join(self.test_dir, 'nir_cloudy', filename), 1).astype(np.float32)
