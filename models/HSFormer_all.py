...
class SparseSamplingAttention(nn.Module):
    ...
    def get_biases(self, x, num_predict_total, window_num_h, window_num_w):
        ...
        sampling_biases = sampling_biases.reshape(num_predict_total, 2, window_num_h,
                                                  window_num_w)  # 2 denotes the x-axis and y-axis
    ...
    def window_self_attention(self, q, k, v, window_num_h, window_num_w, b):
        ...
        k = k.reshape(b, self.num_heads, self.out_dim // self.num_heads, window_num_h, self.ws, window_num_w,
                      self.ws).permute(0, 3, 5, 1, 4, 6, 2).reshape(b * window_num_h * window_num_w, self.num_heads,
                                                                    self.ws * self.ws, self.out_dim // self.num_heads)
        v = v.reshape(b, self.num_heads, self.out_dim // self.num_heads, window_num_h, self.ws, window_num_w,
                      self.ws).permute(0, 3, 5, 1, 4, 6, 2).reshape(b * window_num_h * window_num_w, self.num_heads,
                                                                    self.ws * self.ws, self.out_dim // self.num_heads)
        ...
        out = rearrange(out, '(b h_num w_num) h (ws_h ws_w) d -> b (h d) (h_num ws_h) (w_num ws_w)', h=self.num_heads,
                        b=b, h_num=window_num_h, w_num=window_num_w, ws_h=self.ws, ws_w=self.ws)
        out = self.proj(out)
    ...
    def get_pixel_constraint_matrix(self, x, window_num_h, window_num_w, b, x_residual, h, w):
        # get qkv for self-attention
        qkv = self.qkv(x_residual).reshape(b, 3, self.num_heads, self.out_dim // self.num_heads, h, w).transpose(1,
                                                                                                                 0).reshape(
        ...
        N, _, _, _ = q_k_dots.shape
        _, _, C, _ = q.shape
        mask1 = torch.zeros(N, self.num_heads, C, C, device=x.device, requires_grad=False)
        index = torch.topk(q_k_dots, k=int(C * 4 / 5), dim=-1, largest=True)[1]
        mask1.scatter_(-1, index, 1.)
        attn1 = torch.where(mask1 > 0, q_k_dots, torch.full_like(q_k_dots, float('-inf')))

        attention_map = attn1.softmax(dim=-1)
        out = attention_map @ v

        out = rearrange(out, '(b h_num w_num) h (ws_h ws_w) d -> b (h d) (h_num ws_h) (w_num ws_w)',
                        h=self.num_heads, b=b, h_num=window_num_h, w_num=window_num_w, ws_h=self.ws, ws_w=self.ws)
        constraint_matrix_pixel = self.proj(out)
        constraint_matrix_pixel = self.proj_drop(constraint_matrix_pixel)

        return constraint_matrix_pixel
    ...
    def forward(self, x, var):
        ...
        if self.ranking_constraining:
            var_3d = var.reshape(b, c, h * w)  # .mean(1).unsqueeze(1)
            constraint_matrix_uncertainty = self.get_constraint_matrix(x, var_3d)

        if self.ranking_constraining_pixel:
            constraint_matrix_pixel = self.get_pixel_constraint_matrix(x, window_num_h, window_num_w, b, x_residual, h,
                                                                       w)

        # constraint_matrix = 0.5 * constraint_matrix_pixel + 0.5 * constraint_matrix_uncertainty
        ...
        # aim to regular sampling operator
        # x_var = x * constraint_matrix
        # x_var = x * constraint_matrix_uncertainty
        # get sampling factors
        sampling_biases = self.get_biases(x_var, num_predict_total, window_num_h, window_num_w)
        sampling_scales = self.get_scales(x_var, num_predict_total, window_num_h, window_num_w)
        # transform coords
        ...
        grid_coords = self.get_grid_coords(coords, num_predict_total, window_num_h,
                                           window_num_w)  # coords.permute(0, 2, 3, 4, 5, 1).reshape(num_predict_total, self.ws*window_num_h, self.ws*window_num_w, 2)
        # get qkv for self-attention
        qkv = self.qkv(x_residual).reshape(b, 3, self.num_heads, self.out_dim // self.num_heads, h, w).transpose(1,
                                                                                                                 0).reshape(
            3 * b * self.num_heads, self.out_dim // self.num_heads, h, w)
        ...
        q, k, v = qkv[0], qkv[1], qkv[2]
        # gridsampling
        k_sampling = self.grid_sample_function(
            k.reshape(num_predict_total, self.out_dim // self.num_heads, h + self.shift_size + self.padding_bottom,
                      w + self.shift_size + self.padding_right), grid_coords)
        v_sampling = self.grid_sample_function(
...
class Local_Reconstruction(nn.Module):
    ...
    def self_attention_modulation(self, x, var_modulation):
        ...
        mask2 = torch.zeros(B_, self.num_heads, N, N, device=x.device, requires_grad=False)
        mask3 = torch.zeros(B_, self.num_heads, N, N, device=x.device, requires_grad=False)
        mask4 = torch.zeros(B_, self.num_heads, N, N, device=x.device, requires_grad=False)

        index = torch.topk(q_k_dots_before, k=int(N / 2), dim=-1, largest=True)[1]
        mask1.scatter_(-1, index, 1.)
        attn1 = torch.where(mask1 > 0, q_k_dots_before, torch.full_like(q_k_dots_before, -1e9))

        index = torch.topk(q_k_dots_before, k=int(N * 2 / 3), dim=-1, largest=True)[1]
        mask2.scatter_(-1, index, 1.)
        attn2 = torch.where(mask2 > 0, q_k_dots_before, torch.full_like(q_k_dots_before, -1e9))

        index = torch.topk(q_k_dots_before, k=int(N * 3 / 4), dim=-1, largest=True)[1]
        mask3.scatter_(-1, index, 1.)
        attn3 = torch.where(mask3 > 0, q_k_dots_before, torch.full_like(q_k_dots_before, -1e9))

        index = torch.topk(q_k_dots_before, k=int(N * 4 / 5), dim=-1, largest=True)[1]
        mask4.scatter_(-1, index, 1.)
        attn4 = torch.where(mask4 > 0, q_k_dots_before, torch.full_like(q_k_dots_before, -1e9))
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.refinement = Refine_stage(n_feat=dim, fusion_dim=dim[0], kernel_size=3, reduction=4, act=nn.GELU(),
                                       bias=True,
                                       num_cab=6)  # nn.Sequential(Conv_block(conv_num[0],dim[0],dim[0],expand_ratio=expand_ratio[0],VAN=VAN,dynamic=dynamic_e))

        self.out_final = nn.Conv2d(dim[0], out_cahnnels, kernel_size=3, stride=1, padding=1)
        self.var_conv3 = nn.Sequential(
            *[nn.Conv2d(dim[3], dim[3], 3, 1, 1), nn.ELU(), nn.Conv2d(dim[3], out_cahnnels, 3, 1, 1), nn.ELU()])
        self.var_conv2 = nn.Sequential(
            *[nn.Conv2d(dim[2], dim[2], 3, 1, 1), nn.ELU(), nn.Conv2d(dim[2], out_cahnnels, 3, 1, 1), nn.ELU()])
        self.var_conv1 = nn.Sequential(
            *[nn.Conv2d(dim[1], dim[1], 3, 1, 1), nn.ELU(), nn.Conv2d(dim[1], out_cahnnels, 3, 1, 1), nn.ELU()])
        self.var_conv0 = nn.Sequential(
            *[nn.Conv2d(dim[0], dim[0], 3, 1, 1), nn.ELU(), nn.Conv2d(dim[0], out_cahnnels, 3, 1, 1), nn.ELU()])
        self.var_conv_final = nn.Sequential(
            *[nn.Conv2d(dim[0], dim[0], 3, 1, 1), nn.ELU(), nn.Conv2d(dim[0], out_cahnnels, 3, 1, 1), nn.ELU()])
